...
/*	$.fn.datebox.defaults.formatter = function( date ) {
	...
	$.easyui.openWindow = function( obj ) {
		...
		obj = $.extend({
			...
			resizable: true,
			iframe: true,
			escKey: false, //true则支持Esc按键退出窗口
			sizeConfigurable: false, //true则支持自定义窗口大小
			onOpen: onOpen
		}, obj);
	...
	$.easyui.waitOff = function() {
		if ($.up.$.easyui._waitFlag) {
			$.up.$.easyui._waitFlag.remove();
			delete $.up.$.easyui._waitFlag;
		}
	};
	/**
	 * textbox扩展方法
	...
	$.extend($.fn.textbox.methods, {
		addClearBtn: function(jq, params){
			if ( !params.iconCls ) return;
			return jq.each(function(){
				var iconCls = params.iconCls;
				var t = $(this);
				var eve = params.event || "keyup";
				...
				if (eve === 'hover') {
					...
					t.textbox('textbox').bind(eve, toggleDel);
				}
				function toggleDel() {			
					var icon = t.textbox('getIcon',0);
					if ($(this).val()){
	...
	$.extend($.fn.datagrid.methods, {
		...
		tooltip : function (jq, fields, onlyShowInterrupt) {
			...
			onlyShowInterrupt = onlyShowInterrupt === false ? onlyShowInterrupt : true;
			return jq.each(function () {
				if (fields && typeof fields == 'object' && fields.sort) {
					$.each(fields, function () {
						if ( this !== 'handle' ) { //排出操作列
							bindEvent($('.datagrid-body td[field=' + this + '] .datagrid-cell'), onlyShowInterrupt);
							bindEvent($('.datagrid-header td[field=' + this + '] .datagrid-cell'), onlyShowInterrupt);
						}
					});
				} else {
					bindEvent($(".datagrid-body td:not([field='handle']) .datagrid-cell"), onlyShowInterrupt);
					bindEvent($(".datagrid-header td:not([field='handle']) .datagrid-cell"), onlyShowInterrupt);
				}
			});
	...
	/**
	 * description: buildTree(data) 扁平化加载树, 树形构建 实现扁平化加载树
	 *              该通用方法可能由于require加载exp-easyui.js的原因而失效，
	                最好在使用combotree的js页面自定义loadFilter。
	 * @param data
	 * @returns
	 */
